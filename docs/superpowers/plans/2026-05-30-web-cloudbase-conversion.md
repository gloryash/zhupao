...
- [ ] **Step 1: Write root development scripts**
...
`PublishNeedPage` uses browser geolocation, allows target distance/duration input, and calls `orderApi.publish`. `VolunteerOrdersPage` lists waiting orders and calls `orderApi.accept`. `OrderTrackPage` supports arrived, running, and completed status actions.
...
### Task 15: Claude Code Frontend Design Pass
...
### Task 16: agent-browser Functional And Visual Verification
